void loadUpFromSave(int saveFileNumber){
    ...
    while(strcmp(saveFile[i], "END OF ARRAY") != 0){
        ...
        */
        ...
        char* sModel = customSubString(saveFile[i],point0+1 - saveFile[i],dis1);
        char* sPort = customSubString(saveFile[i],point1+1 - saveFile[i],dis2);
        char* sRigModel =customSubString(saveFile[i],point2+1 - saveFile[i],dis3);
